...
1. :partying_face: Found the solution of last point in [Week 22] which occurs with global :simple-task: Taskfile.
   ...
   - :material-file-document: Related Docs:
     - [:simple-task: Running A Taskfile From A Subdirectory](https://taskfile.dev/usage/#running-a-taskfile-from-a-subdirectory)
     - [:simple-task: Running A Global Taskfile](https://taskfile.dev/usage/#running-a-global-taskfile)
     - [:simple-task: Special Variables](https://taskfile.dev/api/#special-variables)

2. Created :simple-huggingface: [HuggingFace] account because now I want to dive into LLMs and want to create some basic
   application using them and in the flow of creating something I can learn DL and LLM concpets too.
...
1. I saw a guy in :simple-discord: "We write code" Server by Hitesh Chaudhary who has amazing portfolio and he had done
   amazing work around Full Stack Development and he also contributed in multiple Open Source repos (big repos)
   :AMAZING:. He made a full-pledged WORKING clown of PW website and also made a desktop app of PW using Tauri (Rust).
   He has amazing portfolio website. There is so much things I can learn form him. **[@arnvgh]**
2. Completed the refactoring of CTT Model in [@arv-anshul/yt-watch-history-v2] project. See release [v0.3.0].
3. ~~Created a community server on :simple-discord: Discord and added many interactive features which helps others to
   connect and learn together as community.~~
...
5. :star:{: .bounce } A employee from [AABEE](http://aabee.co.in) (a real state startup) reached me through **reached me
   through my OG [99acres-scrape] project**. He conducted a meeting on G-Meet and given a task to scrape the older
   prices of properties to tackle the property's price trend but the data is not there on the website. That's why he
   said "we'll reach you later for any other work" :disappointed:.
6. :star:{: .bounce } The 3 co-founders of [LeapX.ai] startup **reached me through my OG [99acres-scrape] project**.
   They conducted a meeting on MS Teams and the **meeting goes for 48 Minutes**{ title="🤯" } while meeting they ask for
   my education, data science knowledge. In the end, they asked me a problem to explain (not solve; just explain) on the
   basis of explain they offer me a internship :star_struck:.
...
1. :simple-vercel: Project [og-py] to create custom GitHub repo social preview image using
   `https://og-playground.vercel.app/` playground.
2. :star: Working at LeapX.ai as Intern.
3. :llama: Try using `ollama` in :simple-zedindustries: Zed editor. It is great but the response time very bad but I'll
   use it.
4. :simple-playwright: Read/Watch something about Playwright (better alternative of Selenium).
...
: After a one day off, We (Me and Aditya) connect on :material-microsoft-teams: and discuss the affluent properties
clustering problem. I have created some dashboard around it but after the meeting the conclusion is that data is not
good because many properties were removed from 99acres's website. So we decided to scrape new data from the website. So
...
: I am planning to use [`selenium-wire`](https://pypi.org/p/selenium-wire). And, want to scrape more than one website
and create a pipeline through which different websites' data combines together and forms a wholesome dataset to work on.
...
2. Used `st.Page` (new multipage UI update from :simple-streamlit:) while building dashboards for LeapX.
...
: Meeting started after :clock8: **20:00** and end at :clock1030: **22:30**. We worked on dataset scraped from
Housing.com contains ~12K rows.

: We took a different approach to cluster properties, used [polars.Expr.qcut] function to categories properties on the
basis of `"PRICE_PER_UNIT_AREA"`. After this created a dashboard around this (with :simple-streamlit:).

: Now the main problem is to find a dense region and create a circle around it to mark that region with a label _eg.
Luxury, Value, Affordable, etc._{ .secondary } And for this I have to write a function which do these findings and
creating regions.
...
: :butterfly:{ title="Using httpx library" } Scraped all 21K properties from Housing.com website of Gurgaon, faced many
website downtime but eventually got things work.

: :star: Received offer leeter from LeapX.

**2024-06-29**

: :star: Completed the two page streamlit dashboard.

: Told me to learn about AI stuffs like :simple-langchain: Langchain and Llama Index. Already found an awesome video on
:simple-langchain:.
