...
- Created :simple-huggingface: HuggingFace account with `arv@leapx.ai` ID.
- Learned how to use :simple-huggingface: Spaces and created a GH Action in `campaign-dashboard` repo to push code from
  GH to LeapX's HF Space where the streamlit app gets deployed. (done whole thing in 1 Hour :scream:)

**2024-07-12**

- Learned and added Scrapy Pipeline to move scraped data into MongoDB cluster; also added Data Transformation (using
  :simple-polars:) code for scraped data.
...
- `housing-dashboard` is running on :simple-huggingface: HuggingFace Spaces.
...
:star: Building `campaign-bot-api` with @Aditya and @Pratham and almost completed it. Made with :simple-fastapi: FastAPI
with JWT auth (learned on-the-go).

## Week 30 Journal

1. Droping the idea of journaling of LeapX.ai work.
2. Learning :simple-langchain: LangChain and [RAG](https://python.langchain.com/v0.2/docs/tutorials/rag) for internship
   projects.
